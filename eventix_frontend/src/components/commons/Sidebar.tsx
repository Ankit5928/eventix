  ...
  return (
    <aside className="h-screen w-64 bg-gray-900 text-white flex flex-col fixed left-0 top-0 z-50">
      ...
      <div className="p-6 border-b border-gray-800">
        <Link to="/dashboard" className="text-2xl font-bold tracking-tight text-blue-500">
          EVENTIX<span className="text-white text-xs ml-1 uppercase">Admin</span>
        </Link>
      </div>
      ...
      <nav className="flex-1 mt-6 px-4 space-y-1">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={`w-5 h-5 mr-3 ${isActive ? 'text-white' : 'group-hover:text-blue-400'}`} />
